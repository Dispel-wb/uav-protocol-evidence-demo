(function () {
  ...
  function renderStream() {
    const all = [...report.frames.map(f => ({...f, segmentType:'frame'})), ...report.gaps.map(g => ({...g, segmentType:'gap'}))].sort((a,b) => a.offset-b.offset);
    const track = all.map(x => {
      const percent = Math.max(4, x.length / report.bytes * 100);
      const cls = x.segmentType === 'gap' ? 'gap' : x.family === 'SBUS' ? 'sbus' : x.crc === 'invalid' ? 'error' : '';
      const title = x.segmentType === 'gap' ? `未识别 ${x.length} B` : `${x.family} · ${x.name} · ${x.length} B`;
      const shortName = x.segmentType === 'gap' ? '…' : x.family === 'SBUS' ? 'SBUS' : ({HEARTBEAT:'HB',GPS_RAW_INT:'GPS',ATTITUDE:'ATT',COMMAND_LONG:'CMD',COMMAND_ACK:'ACK'}[x.name] || `#${x.messageId}`);
      return `<div class="segment ${cls}" style="flex:${x.length} 1 0;max-width:${Math.max(percent,7)}%" title="${safe(title)}">${safe(shortName)}</div>`;
    }).join('');
    $('stream').innerHTML = `<div class="stream-caption">字节分段 · 按偏移排列（总长 ${fmt(report.bytes)} B）</div><div class="stream-track">${track}</div><div class="stream-legend"><span><i></i>MAVLink</span><span><i class="s"></i>SBUS</span><span><i class="e"></i>校验错误</span></div>`;
  }
  function showFrame(index) {
    ...
    const fields = frame.family === 'SBUS' ? { channelsRaw: frame.channels, frameLost: frame.frameLost, failsafe: frame.failsafe, digital17: frame.digital17, digital18: frame.digital18 } : (frame.fields || {});
  ...
  function render() {
    ...
    $('m-bytes').textContent = fmt(s.totalBytes); $('m-frames').textContent = s.frames;
    $('m-mav').textContent = s.mavlink; $('m-sbus').textContent = s.sbus; $('m-findings').textContent = s.findings;
    $('parse-status').textContent = `${s.frames} 帧 · ${report.gaps.length} 段未识别`;
    renderStream();
    $('frame-rows').innerHTML = report.frames.map((f,i) => {
    ...
    if (report.frames.length) showFrame(0);
  ...
  function download() {
    if (!report) return;
    const payload = { schema:'protocol-lab-report-v1', generatedAt:new Date().toISOString(), inputHex:decoder.hex(currentBytes), ...report };
