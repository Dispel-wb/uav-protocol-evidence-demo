...
BPSK与QPSK现在同时尝试矩形脉冲的积分判决和滚降系数0.35的根升余弦匹配滤波判决；载波频偏由平方／四次谱线峰值估计，避免成形过渡区的低幅相位使相位展开失稳。`npm run evaluate:rrc` 对两种调制、3档噪声和3档频偏执行18组合成测试，18组均恢复正确载荷并通过MAVLink CRC，错误调制选择和错误CRC接收均为0。完整结果位于 `reports/rrc_psk_evaluation.json`。发射端和接收端仍使用同一套合成实现，尚未覆盖连续变化的采样漂移、多径、功放非线性和真实发射机。

RRC分支进一步在−10000～+10000 ppm的离散候选上改变符号采样周期，并用同步字误差和星座离散度排序。`npm run evaluate:clock-drift` 对BPSK/QPSK各5组时钟偏移做消融：固定符号时钟仅精确恢复4/10组，启用漂移搜索后恢复10/10组，改善6组、回退0组，错误调制和错误CRC接收均为0。该范围用于在短突发中放大漂移效应；当前实现是有限候选搜索，还不是可跟踪连续漂移的Gardner定时环。报告位于 `reports/psk_clock_drift_ablation.json`。
